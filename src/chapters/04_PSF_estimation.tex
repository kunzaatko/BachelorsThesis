...
\end{gather*}
Furthermore if we do not want to limit to theorized distribution of noise, we can eyeball or even make a histogram,
    kernel of another empirical estimate of the distribution of the intensities in \(A\).
...
\begin{marginfigure}
    ...
    \caption{The Ostu threshold leads to lower intensities of some beads to be passed as background. On the left, side,
        Otsu threshold (\(\approx 0.13\)) is used and on the right, the it is lowered by \(0.07\) to \(\approx 0.6\). The top
        image shows the difference of the two on a larger area.}
...
The problem therefore reduces to the distinction of the beads from the background for which an Otsu
    threshold\sidecite{otsu1979} can be used however it is necessary to lower the threshold to reduce the lower
    intensities\reffig{beads-leakage-on-otsu-threshold} that are due to the beads fluorescence.

\begin{figure}[h]
    \png{noise-distribution-hist-from-background}
...
If we consider the background noise as the true noise distribution, we can estimate the \ac{SNR}\sidenote{A popular
    metric in optical sensing.} of the image as well.
