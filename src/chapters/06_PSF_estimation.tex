...
\begin{description}
    ...
    \item[Photon noise]
        ...
        Since \(\E N = \Var(N) = \lambda'\Delta\tau \eqqcolon \lambda\), the ratio of signal to noise increases 
...
The comparison with the narrow mean \(\reconstructed_{\text{n}}\) is performed in order to discard the false 
    positive locations that are present because of the reconstruction artefacts that occur using the deconvolution 
    reconstruction~(\refsecshort{deconvolution-reconstruction}) and to discard the pixels connecting \acp{ROI} of beads
