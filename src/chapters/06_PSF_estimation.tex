...
If we have an optical system and want to model the way light passes through that system, either in order to enhance our
    ...
    need to determine one of the transfer functions of the optical system. % introduced in \refch{transfer-functions}. % TODO: reference <14-11-23> 
...
\section[Image Formation]{Image Formation Model}%
...
\subsection{Locating the Beads (\refitmshort{locations-of-beads})}%
\labsubsec{locating-the-beads}
...
\section[New Reconstruction]{Revised Reconstruction}%
\labsec{revised-reconstruction}


There is a possibility of enhancing the reconstruction at a give location by using the transfer function from that
    location instead of a model estimate from the whole image plane.
In this spirit the image was subdivided into patches of right-top, left-top, center-center, right-bottom and left-bottom
    in the \(xy\)-plane.
\(12\) beads were selected from each patch within the image.
An averaging estimation of the \ac{PSF} was performed and the Wiener filter reconstruction of the full image plane was 
    made using the localized \ac{PSF}.

\begin{figure}
    \pdftex{avg-location-psf-reconstructions-spatial-comp}
    \caption{
        The reconstructions of the image using the localized \ac{PSF} estimates.
        Boxes indicate the patches, where the beads, were selected from for the \ac{PSF} estimate.
    }
    \labfig{avg-location-psf-reconstructions-spatial-comp}
\end{figure}

\begin{figure}
    \pdftex{avg-location-psf-reconstructions-spatial-comp-rt}
    \caption{
        Comparison of the left-bottom patch using the reconstructions of the image using the localized \ac{PSF} 
            estimates.
        There is a slight visible difference near the edges of the beads.
    }
    \labfig{avg-location-psf-reconstructions-spatial-comp-rt}
\end{figure}

There are perceivable differences between the patch reconstructions 
    (\reffigshort{avg-location-psf-reconstructions-spatial-comp}) using the \acp{PSF} estimates from the patches.
In general, however, the reconstruction of the image with the patch estimate of the \ac{PSF} at the patch indices is
    noticeably better than those with \ac{PSF} estimate other than that particular patch 
    (\reffigshort{avg-location-psf-reconstructions-spatial-comp-rt}).
It is logical and true that the center patch estimate of the \ac{PSF} produces reconstructions that are best in average
    over the whole image plane, since the aberrations are closest to the off-center ones in average.

The next step is to compare the reconstruction using the model and the estimate.
This is done in \reffig{model-vs-cc-reconstruction}, where the reconstruction using the center-center patch \ac{PSF}
    estimate is shown.
A resolution and visual enhancement is undoubtedly achieved using this reconstruction.

\begin{figure}
    % \png{avg-location-psf-reconstructions-f-comp-cc}\\[\medskipamount]
    % \png{avg-location-psf-reconstructions-s-comp-cc}
    \pdftex{avg-location-psf-reconstructions-b-comp-cc}
    \caption{Comparison of the reconstructions and their Fourier transforms made using the model \ac{OTF} and using the
        center-center patch localized averaged\ac{PSF}.
        There is a significant difference in the uniformity of the Fourier image.
        Moreover, there are no visible peaks in the centres of the components.
        This reduces the artefacts that are caused by the incorrect transfer function significantly.
    }
    \labfig{model-vs-cc-reconstruction}
\end{figure}

We also want to see, if the estimated transfer function is useful for the reconstruction of acquisition of the cell,
    since this is the prime goal of making these estimates.
To asses the quality of the reconstruction, we use the center-center patch \ac{PSF} and the model \ac{OTF} to 
    reconstruct cell image \ac{SIM} acquisition using the same microscope (\reffigshort{comp-cell-cc-model}).

\begin{figure}[h!]
    \pdftex{avg-location-psf-reconstructions-b-comp-cc-ccp}
    \caption{%
        A comparison of the reconstruction of the cell acquisition made by the model and the center-center patch
            estimate of the \ac{PSF}.
        A visual quality enhancement is achieved, although based on the Fourier space results, an even better result 
            might be possible by using apodization or limiting estimating the support of the \ac{OTF} produced by the 
            \ac{PSF} and artificially reducing it to \(0\) out of the bound of the support.
}
    \labfig{comp-cell-cc-model}
\end{figure}
\vfill

\pagebreak

The quality improvement is also undeniable, so we may conclude, that the \ac{PSF} estimate is more adequate for the cell
    images as well (which is logical).
For better comparison of the artefact elimination, a closer view of the reconstruction is shown in 
    \reffig{comp-cell-cc-model-donut}.

\begin{figure}[h!]
    \pdftex{avg-location-psf-reconstructions-b-comp-cc-ccp-donut}
    \caption{%
        A close up of the spatial result achieved using the model and the center-center \ac{PSF} estimate.
        Although the artefacts are not fully eliminated a reduction is evident.
    }
    \labfig{comp-cell-cc-model-donut}
\end{figure}

\section{Discussion}%
\labsec{discussion}

Although significant improvement is achieved there is a vast opportunity for improvement.
Some of the improvements may be made to the estimation of the \ac{PSF} itself, but there is also room for enhancing the
    reconstruction algorithm (Wiener filter is used) to match the attributes of the \ac{PSF} estimate.

First for the estimation of the \ac{PSF}.
\begin{enumerate}
   \item%
       As has been indicated in \refsubsec{noise}, the model of the noise is truly the one and discussed in the subsection.
        There are methods for fitting the model, one of which was cited and commented upon in the subsection.
        If this fit was to be made, we could calculate the distribution of the estimate of the averaged bead.
        This could give us the opportunity to optimize the transfer function further, based on the artefacts that are produced
            by the reconstruction employing it by defining some loss related to the amount of artefacts produced.
        This optimization could be made with the knowledge of how probable it is for the \ac{PSF} to truly be this optimized 
            form in contrast to the base estimate.
    \item%
        The source model estimation, or more specifically the \refsubsec{locating-the-beads} lead to finding bead
            locations that were merely artefacts. 
        This could be eliminated by developing a locating algorithm on the \ac{LR} summed image, which does not include
            artefacts and the resulting locations could be compared and further filtered to eliminate all the artefacts.
        It, however, is not possible to distinguish some artefacts from true beads even by eye, so an ideal model is not
            thinkable using this approach.
    \item%
        The \ac{PSF} metric of \ac{FWHM} has been found to be different for the center and edge estimated \acp{PSF}.
        This is not a surprise, since we know that the edge locations are more affected by the aberrations.
        However, it proposes a possibility of determining the aberrations through assessing the \ac{OTF} produced by the
            measured \ac{PSF} through fitting of Zernicke polynomials \sidecite{siddik2023a} and through this, it may be 
            possible to produce a model of the location variant aberrations strength.
        This could be utilized to determine a location variant model of the transfer function and improve the model
            reconstruction.
\end{enumerate}

More importantly, what is possible to improve in the reconstruction in order of assumed importance:
\begin{enumerate}
    \item%
        Limiting the support of the \ac{OTF} that is produced by the \ac{PSF} which should further reduce the
            artefacts.
    \item%
        Seeing from the Fourier space reconstructions, that the edges of the components are still have a visibly
            higher intensity than the centres of the components, this could be alleviated by using apodization, which 
            should further reduce the artefacts.
    \item%
        The \ac{FWHM} in \(x\) and \(y\) axes differed significantly in all the estimations except for the center-center 
            patch estimate.
        This means that the spherical and coma are present and that the aplanatic assumption is wrong by a greater
            margin then is expected.
        The size of the error of the aplanatic assumption is a clear indication, that tiled reconstruction could
            markedly improve the reconstructions.
        There is also the possibility of an even better approach than the tiles produced by the estimate from the
            averaging of beads throughout some predefined patches of the image plane.
        It is in the assessment and integration of the estimate from \refsec{estimating_the_psf_from_the_model}, which
            could produce the estimate for any given sufficiently large window within the image.
    \item%
        It is visible that the center components are have a greater intensity than the edge components by
        a significant amount which is not realistically the true ratio of intensities. 
        This could be more closely analysed by the Fourier transform of the \ac{LR} image sum, as there is some overlap
            between the components.
        This indicates the wrong estimation of modulations of the illumination patterns.
        Therefore the modulation estimate should be modified and the resulting reconstruction of those modification
            should be monitored to asses its influence.
        If we had a measure for the artefacts, it would even be possible to optimize modulations based on the 
            reconstruction artefacts, it produces.
\end{enumerate}
