...
The following sections will demonstrate the deconvolution based approach 
    (\refsecshort{deconvolution-reconstruction}) and then the inversion/optimization based approach 
...
The general form of the \ac{LR} image illumination patterns in one orientation, denoted \(\ip_{\phi_1}\), 
    \(\ip_{\phi_2}\) and \(\ip_{\phi_3}\), that are at the input of the deconvolution reconstruction is
    \begin{equation}
        \labeq{ip-formula}
...
\section{Deconvolution Reconstruction}
\labsec{deconvolution-reconstruction}

The deconvolution approach consists of:
\begin{enumerate}[label=\roman*.]
    ...
    \item \labitm{otf-shift} determining the \ac{OTF} of the mapped components and the \ac{HR} image 
        ...
        \end{equation*}
    \item \labitm{deconvolution} deconvolution of the mapped components 
        \begin{equation*}
...
\marginnote{The necessity of the separation of components is one of the major drawbacks for the deconvolution approach,
...
the reconstruction using the deconvolution approach. It would however be possible to use the other two methods.}
...
That is, an estimate of the source signal from the information contained in the component \(\Fcomponent'_{\vec{p}}\) can
    ...
    \refsubsec{deconvolution}.} written as
    \begin{equation}
...
\subsection{Deconvolution (\refitmshort{deconvolution})}
\labsubsec{deconvolution}
...
It is derived in as a least square error estimate of the original signal, given a noisy sensed signal and a known
    transfer (degradation) function (\refch{deconvolution}).
...
The principle of the inversion reconstruction is in many ways easier compared to the deconvolution reconstruction
    (\refsecshort{deconvolution-reconstruction}).
...
Unlike the deconvolution optimization, we do not attempt to develop an estimate for \(\fwd^{-1}\), but instead, it is
    sufficient to formulate the forward model and rely on mathematical optimization for estimating the correct desired
    input \(\source\), minimizing some defined loss \(\loss\) between \(\fwd[\source]\) and the sensed \(\sensed\).
...
The numerical optimization itself is performed using gradient descent with the use of source-to-source backward 
    propagated automatic differentiation for computing the gradient and hessian.
...
An aspect that must be considered when choosing a reconstruction method, if the object of the observation is not static 
    ...
    observation.
There is an intrinsic deficiency of the deconvolution reconstruction in the fact that it is necessary to sense the image
